...
class robot:
    ...
    def move(self, motion, computeWithNoise = 1): # Do not change the name of this function
        ...
        if computeWithNoise:
            steeringAngle = motion[0] + self.Gaussian(0, result.steering_noise, motion[0])
            distanceOfTravel = motion[1] + self.Gaussian(0, result.distance_noise, motion[1])
        else:
            steeringAngle = motion[0]
            distanceOfTravel = motion[1]
        turningAngle = 0.0
        turningRadius = 0.0
        cX = 0.0
        cY = 0.0
        #Restricting the maximum steering angle to pi/4 defined in global
        if steeringAngle > 0 and abs(steeringAngle)/max_steering_angle > 1:
            steeringAngle = max_steering_angle
        elif steeringAngle < 0 and abs(steeringAngle)/max_steering_angle > 1:
            steeringAngle = (-1)*max_steering_angle
        
        if abs(steeringAngle) < 0.001:
            #Robot is not turning
            result.x = self.x + distanceOfTravel*cos(self.orientation)
            result.y = self.y + distanceOfTravel*sin(self.orientation)
            result.orientation = self.orientation%(2*pi)
        else:
            #Robot is turning with some steering angle
            turningAngle += (distanceOfTravel/self.length)*tan(steeringAngle)
            turningRadius += distanceOfTravel/turningAngle
            cX += self.x - sin(self.orientation)*turningRadius
            cY += self.y + cos(self.orientation)*turningRadius
            result.x = cX + sin(self.orientation + turningAngle)*turningRadius
            result.y = cY - cos(self.orientation + turningAngle)*turningRadius
            result.orientation = (self.orientation + turningAngle)%(2*pi)
    ...
    def sense(self, computeWithNoise = 1): #do not change the name of this function
        Z = []
        deltaX = 0.0
        deltaY = 0.0
        ...
        for i in range(len(landmarks)):
            #Since landmarks are in y,x format
            deltaX = landmarks[i][1] - self.x
            deltaY = landmarks[i][0] - self.y
            rawBearing = atan2(deltaY, deltaX)
            bearing =  (rawBearing if rawBearing >= 0 else (rawBearing + 2*pi))\
                      - self.orientation
            if computeWithNoise:
