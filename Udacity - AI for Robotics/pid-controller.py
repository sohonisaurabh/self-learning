...
def run(param1, param2, param3):
    ...
    # Enter code here
    #
    prevCTError = myrobot.y
    totalCTError = 0.0
    for i in range(N):
        currentCTError = myrobot.y
        diffCTError = currentCTError - prevCTError
        totalCTError += currentCTError
        steering = (((-1)*param1*currentCTError) + ((-1)*param2*diffCTError) + ((-1)*param3*totalCTError))
        myrobot = myrobot.move(steering, 1)
        print myrobot.__repr__()
        prevCTError = currentCTError
