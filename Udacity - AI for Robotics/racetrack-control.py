...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #
        #
        # Boundary definition goes by the pattern: Left, Top, Right, Bottom
        boundaryDefn = [[0, 0], [0, 2*radius], [100, 0], [0, 0]]
        direction = self.orientation
        print direction
        if direction <= pi/2 and direction > 0:
            cte = self.y - boundaryDefn[1][1]
            print "Going left to top"
        elif direction <= 0 and direction > (-1)*pi/2.0:
            cte = self.x - boundaryDefn[2][0]
            print "Going top to right"
        elif direction <= (-1)*pi/2.0 and direction > (-1)*pi:
            cte = self.y - boundaryDefn[3][1]
            print "Going right to bottom"
        elif direction <= (-1)*pi and direction > (-1)*3*pi/2.0:
            cte = self.x - boundaryDefn[0][0]
            print "Going bottom to left"
        else:
...
def run(params, radius, printflag = False):
    ...
    for i in range(5):
         ...
         print "CTE is:" + str(crosstrack_error)
