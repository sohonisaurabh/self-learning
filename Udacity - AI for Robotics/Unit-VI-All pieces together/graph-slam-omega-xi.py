...
def doit(initial_pos, move1, move2):
    ...
    #
    #
    #Initialized omega here
    omega = matrix()
    omega.zero(3, 3)
    #Initialized xi here
    xi = matrix()
    xi.zero(3, 1)
    motionArray = [initial_pos, move1, move2]

    #Initializing omega and xi with intial robot position
    ...
    for i in range(2):
        ...
        tempXi = matrix()
        tempXi.zero(3, 1)
        tempMat.value[i][i+1] = -1.0
        tempMat.value[i][i] = 1
        tempMat.value[i+1][i+1] = 1
        tempMat.value[i+1][i] = -1.0
        tempXi.value[i][0] = -1*motionArray[(i+1)%3]
        tempXi.value[(i+1)%3][0] = motionArray[(i+1)%3]
        omega = omega.__add__(tempMat)
        xi = xi.__add__(tempXi)
