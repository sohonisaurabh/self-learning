...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    def getNeighbors(node):
        ...
        for i in range(len(direction)):
            currentNeighbor = [node[0] + direction[i][0], node[1] + direction[i][1]]
            if (((currentNeighbor[0] >= 0) and (currentNeighbor[0] < len(road))) and \
               ((currentNeighbor[1] >= 0) and (currentNeighbor[1] < len(road[0])))):
                neighbors.append(currentNeighbor)
        return neighbors

    def filterVisitedNodes(neighborNodes, visitedNodes):
        filteredNodes = []
        for i in range(len(neighborNodes)):
            if (neighborNodes[i] not in visitedNodes):
                filteredNodes.append(neighborNodes[i])
        return filteredNodes

    def getDirection(currentNode, nextProbableNode):
        directionName = ""
        diffX = currentNode[0] - nextProbableNode[0]
        diffY = currentNode[1] - nextProbableNode[1]
        if (diffX == 0 and diffY == 0):
            directionName = "no motion"
        else:
            directionName = direction_name[direction.index([diffX, diffY])]
        return directionName
    
    def isLaneChangeRequired(nodeWithMetaData):
        currentNode = nodeWithMetaData[2]
        nextProbableNode = nodeWithMetaData[1]
        if (getDirection(currentNode, nextProbableNode) == "up" or
            getDirection(currentNode, nextProbableNode) == "down"):
            return True
        else:
            return False

    def assignCost(nodeWithMetaData):
        if (isLaneChangeRequired(nodeWithMetaData)):
            cost = (1.0/road[nodeWithMetaData[2][0]][nodeWithMetaData[2][1]]) + costMap[nodeWithMetaData[1][0]][nodeWithMetaData[1][1]] + lane_change_cost
##            print "Lane change required"
##            print cost
        else:
            cost = (1.0/road[nodeWithMetaData[2][0]][nodeWithMetaData[2][1]]) + costMap[nodeWithMetaData[1][0]][nodeWithMetaData[1][1]]
##            print "Lane change not required"
##            print cost
        return cost
    ...
    direction = [[-1,  0], # go up
           ...
           [ 1,  0], # go down
           [ 0,  1]] # do right
    direction_name = ['up', 'left', 'down', 'right']
    neighborNodes = []
    visitedNodes = []
    taggingCounter = 0
    openNodes = [[taggingCounter, goal, goal]]
    costMap = [[1000 for col in range(len(road[0]))] for row in range(len(road))]
    costMap[goal[0]][goal[1]] = 0
    costForNode = 1000
    i = len(road) - 1
    j = len(road[0]) - 1

##    currentNode = openNodes[0][2]
##    visitedNodes.append([currentNode])
##    neighborNodes = getNeighbors(currentNode)
##    neighborNodes = filterVisitedNodes(neighborNodes, visitedNodes)
##    for i in range(len(neighborNodes)):
##        openNodes.append([taggingCounter, currentNode, neighborNodes[i]])
##    if (assignCost(openNodes[0]) < costMap[currentNode[0]][currentNode[1]]):
##        costMap[currentNode[0]][currentNode[1]] = assignCost(openNodes[0])
##    print costMap

    for i in xrange(len(road) - 1, -1, -1):
        for j in xrange(len(road[0]) - 1, -1, -1):
            currentNode = [i, j]
            neighborNodes = getNeighbors(currentNode)
            for k in range(len(neighborNodes)):
                openNodes.append([taggingCounter, currentNode, neighborNodes[k]])
            j -= 1
        i -= 1
    print openNodes
    print len(openNodes)
...
##            openNodes.append([taggingCounter, currentNode, neighborNodes[i]])
##
        costForNode = assignCost(openNodes[0])
        print currentNode
        print costMap[currentNode[0]][currentNode[1]]
        print costForNode
        print "================"
        if (costForNode < costMap[currentNode[0]][currentNode[1]]):
            costMap[currentNode[0]][currentNode[1]] = costForNode
        openNodes.pop(0)
    print taggingCounter
...
plan(test_road1, lane_change_cost1, test_init1, test_goal1)
